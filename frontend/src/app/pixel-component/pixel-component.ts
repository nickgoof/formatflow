...
export class PixelComponent implements OnInit, OnDestroy {
  ...
  onUpscale(): void {
    if (this.selectedFile) {
      ...
      // Das Format aus dem Dateinamen extrahieren (z.B. "bild.png" -> "png")
      // Falls das fehlschlägt, nehmen wir standardmäßig "jpg"
      const format = this.selectedFile.name.split('.').pop() || 'jpg';

      this.ffservice.upscaleImage(this.selectedFile, format).subscribe({
        next: (blob: Blob) => {
  ...
  onDownscale(): void {
    if (this.selectedFile) {
      ...
      // Das Format aus dem Dateinamen extrahieren
      const format = this.selectedFile.name.split('.').pop() || 'jpg';

      this.ffservice.downscaleImage(this.selectedFile, this.compressionFactor, format).subscribe({
        next: (blob: Blob) => {
          const downloadLink = document.createElement('a');
          downloadLink.href = URL.createObjectURL(blob);
